...
export default function () {
  ...
  check(balancesResponse, {
    ...
    'get balances returns account array': (response) => Array.isArray(response.json()),
  });
  ...
  check(listOrdersResponse, {
    ...
    'list orders contains created order': (response) => response.json().some((order) => order.id === orderId),
  });
