...
func TestCalculateTradeSettlement_BuyerReceivesBTC(t *testing.T) {
	// Arrange
	svc := NewExchangeService(nil, nil, nil, nil, nil, "BTC-USD", nil, nil)
...
func TestRestoreEngineSnapshot_Success_RebuildsActiveOrders(t *testing.T) {
	...
	svc := NewExchangeService(orderRepo, accountRepo, tradeRepo, &MockUserRepository{}, &MockDBTransaction{}, "BTC-USD", nil, nil)
