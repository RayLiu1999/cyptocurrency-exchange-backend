...
### Q5：如何發現並解決系統中的「隱形瓶頸」？
> **回答**：
> 「在壓測初期，我們發現 RPS 下降但 CPU 卻很空閒。我透過對 `pgxpool` 實作 Custom Metrics 監控連線池狀態，發現瓶頸在於 **Wait Count** 指標急遽上升——即服務在『等待』資料庫連線而非處理邏輯。我隨即調整了 Min/Max Connections，並優化了 `FOR UPDATE` 的持有時間，RPS 立即提升了 40%。這證明了**深度觀測力 (Deep Observability)** 在高併發系統中的重要性。」

### Q6：本地壓測的數據能代表線上嗎？

> **回答**（這是展現資深視野的絕佳機會）：
>
> 「本地端的壓測數據（例如 5,000 QPS）放到線上環境並**沒有**絕對的絕對數值參考價值，因為雲端的網路拓撲、Load Balancer 行為都不同。但它有一個無可取代的價值：**『驗證程式碼架構的理論極限』**。
>
> 在本地壓測時，我刻意**移除所有 Docker 的 CPU 與 Memory 限制**。這不是為了模擬線上，而是為了知道：『當硬體不是瓶頸時，我的架構瓶頸會出現在哪？』。
> 透過這種方式，我能證明即使打到極限，我的 Go 程式和 in-memory 計算都不會卡死，最後的瓶頸是純粹的 I/O 與 Database 寫入。這向團隊證明了：如果線上需要支援百萬 TPS，我們該優化的是 Database Sharding 或是 Kafka 配備，而不是微服務本身的程式碼邏輯。」
>
> 另外，在部署到雲端（如 AWS ECS）後，我會重新加上嚴格的資源限制（如 1 CPU / 1GB RAM），那時候測出的則是「真實環境的基線成本 (Baseline)」。
### Q6：壓測時「資料量大小」會影響瓶頸嗎？
...
> 2. 當行情更新過快超過客戶端接收能力時，採用 **Conflation (合流)** 策略只保留最新快照。
> 3. 對於持續堆積緩衝區的連線，系統會根據背壓 (Backpressure) 指標主動斷開，防止單一慢速用戶導致全節點 OOM。
