...
- [v] **本地測試 (Local Testing)**: 使用 k6 或其他工具在本地進行高併發壓測，觀察 Redis 緩存與 Kafka Message Queue 的吞吐量與延遲。
- [v] **ECS 測試部署**: 將服務短期部署至 AWS ECS 進行真實環境的壓力測試。此環境主要用於驗證雲端網路、IAM 權限與基礎設施效能。
- [ ] **瓶頸分析與調優**: 找出系統的效能瓶頸（如 Database 連線數、Kafka 消費速度）並進行調整。

## Phase 2: VPS K3s 基礎部署 (Mid-term)
**目標：將系統從短暫的 ECS 測試環境轉移至長期穩定、低成本的 VPS K3s 叢集。**
- [v] **K3s 集群建置**: 在 VPS 上搭建並配置單節點或多節點的 K3s。
- [v] **基礎設施遷移**: 透過 Helm/Manifest 將 Redis, Kafka, PostgreSQL 等有狀態服務部署或連接至 K3s。
- [v] **微服務遷移**: 將交易引擎、API Gateway 等無狀態服務以 Deployment 形式運行於 K3s。
- [v] **CI/CD 建置**: 設定自動化部署流程，當程式碼推進時自動建置 Image 並更新至 K3s 集群。
...
- [v] **可觀測性建置 (Observability)**: 部署 Prometheus, Grafana, Alertmanager 監控 K3s 節點與各微服務健康度。
